...
from microstack_init.questions.question import Question, InvalidAnswer
...
from oslo_serialization import base64, msgpackutils
...
class Role(Question):
    ...
    _question = (
        "Which role would you like to use for this node:"
        ' "control" or "compute"?'
    )
    ...
    def _input_func(self, prompt):
        ...
        for _ in range(0, 3):
            ...
            print(
                'The role must be either "control" or "compute".',
                file=sys.stderr,
            )
...
class ConnectionString(Question):
    ...
    _question = (
        "Please enter a connection string returned by the"
        " add-compute command > "
    )
    interactive = True
    ...
    def _validate(self, answer: str) -> Tuple[str, bool]:
        ...
        try:
            ...
            logger.debug(
                "The hostname specified in the connection string is"
    ...
    def _validate_hostname(self, hostname):
        ...
        if not re.search("[a-zA-Z-]", name.split(".")[-1]):
            raise ValueError(
                f"{hostname} contains no non-numeric characters"
    ...
    def _validate_fingerprint(self, fingerprint):
        ...
        if not actual_len == expected_len:
            raise ValueError(
                "The provided fingerprint has an invalid "
    ...
    def _validate_credential_id(self, credential_id):
        ...
        if actual_len != UUID_LEN:
            raise ValueError(
                "The credential length is not equal to a length"
    ...
    def _validate_credential_secret(self, credential_secret):
        ...
        if actual_len != SECRET_LEN:
            raise ValueError(
                "The credential secret has an unexpected length:"
